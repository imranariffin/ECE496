...
class FlaskrTestCase(unittest.TestCase):
  ...
  def test_rating_should_give_average_rating(self):
    with app.app.test_client() as c:
      sitter_username = "imran"
      username = "parent1"

      res = c.get("/api/babysitter/%s/rating"%sitter_username)
      rating = json.loads(res.data)
      assert res.status_code == 200
      assert 'rating' in rating
      if rating['rating'] is not None:
        assert rating['rating'] >= 0 and rating['rating'] <= 5

  def test_rating_should_respond_0_for_host_with_no_rating(self):
    with app.app.test_client() as c:
      sitter_with_no_rating_username = "host2"
      username = "parent1"

      res = c.get("/api/babysitter/%s/rating"%sitter_with_no_rating_username)
      rating = json.loads(res.data)
      assert res.status_code == 200
      assert 'rating' in rating
